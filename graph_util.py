...
def markov_blanket(graph, id):
  """Returns the Markov blanket of a node.
   ...
   The Markov blanket of a node is defined as the node's parents, children, and
   parents of children. The node itself is excluded.
...
def neighbor_map(graph, directed=True):
  ...
  :param directed: optional argument (default True) that specifies whether edges
  should be treated as directed or bi-directional.
...
def is_connected(graph, start_id, end_id, directed=True):
  ...
  :param graph: a graph_types.GraphType containing information about the graph.
